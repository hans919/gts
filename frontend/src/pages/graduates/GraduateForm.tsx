...
export default function GraduateForm() {
  ...
  const fetchGraduate = async () => {
    try {
      ...
      setError('Failed to load graduate data');
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.data?.errors) {
        ...
        setError(err.response?.data?.message || 'Failed to update graduate');
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {error && (
          <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
            <p className="text-sm font-medium text-destructive mb-2">Error updating graduate:</p>
